...
async function sendNotificationEmail(payload: FeedbackRequestPayload, requestId: string) {
  const graphAccessToken = await getGraphAccessToken();
  const senderUserId = Deno.env.get('MS_SENDER_USER_ID');
  const recipient = Deno.env.get('FEEDBACK_MAILBOX_ADDRESS');

  if (!senderUserId || !recipient) {
    throw new Error('Faltan secretos FEEDBACK_MAILBOX_ADDRESS o MS_SENDER_USER_ID.');
  }

  const html = `
    <h2>Nueva reclamacion, sugerencia o felicitacion</h2>
    ...
    <p><strong>Motivo:</strong> ${payload.subject}</p>
    <p><strong>Comentario:</strong> ${payload.comments}</p>
  `;
  ...
  const graphResponse = await fetch(
    `https://graph.microsoft.com/v1.0/users/${senderUserId}/sendMail`,
    {
      ...
      body: JSON.stringify({
        message: {
          subject: `Nuevo comentario de cliente - ${payload.company}`,
          body: {
            contentType: 'HTML',
            content: html,
          },
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const rawPayload = (await request.json()) as FeedbackRequestPayload;
    const payload: FeedbackRequestPayload = {
      ...
      subject: requireField(rawPayload.subject, 'Motivo'),
