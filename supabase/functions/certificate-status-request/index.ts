interface CertificateStatusRequestPayload {
  name: string;
  company: string;
  phone: string;
  email: string;
...
async function sendNotificationEmail(
  ...
  const recipient = Deno.env.get('STATUS_MAILBOX_ADDRESS');

  if (!senderUserId || !recipient) {
    throw new Error('Faltan secretos STATUS_MAILBOX_ADDRESS o MS_SENDER_USER_ID.');
  }

  const html = `
    <h2>Nueva solicitud de estatus de certificado</h2>
    ...
    <p><strong>Titular:</strong> ${payload.holder}</p>
    <p><strong>No. de certificado:</strong> ${payload.certificateNumber}</p>
    <p><strong>Fecha de emision:</strong> ${payload.issueDate}</p>
    <p><strong>Norma / disposicion tecnica:</strong> ${payload.standard}</p>
  ...
  const graphResponse = await fetch(
    `https://graph.microsoft.com/v1.0/users/${senderUserId}/sendMail`,
    {
      ...
      body: JSON.stringify({
        message: {
          subject: `Nueva solicitud de estatus de certificado - ${payload.company}`,
          body: {
            contentType: 'HTML',
            content: html,
          },
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const rawPayload = (await request.json()) as CertificateStatusRequestPayload;
    const payload: CertificateStatusRequestPayload = {
      ...
      holder: requireField(rawPayload.holder, 'Titular'),
      certificateNumber: requireField(rawPayload.certificateNumber, 'No. de certificado'),
      issueDate: requireField(rawPayload.issueDate, 'Fecha de emision'),
      standard: requireField(rawPayload.standard, 'Norma'),
